...
export async function generateMetadata({
	...
	console.log("params from generate meta >>, " ,await  params);
	console.log("searchParams from generate meta >>, " , await searchParams);
	
	const { permalink } = await params;
	const searchParamsResolved = await searchParams;
	const permalinkSegments = permalink || [];
	console.log("permalinkSegments >>", permalinkSegments);
	
	const resolvedPermalink = `/${permalinkSegments.join('/')}`.replace(/\/$/, '') || '/';
	console.log("resolvedPermalink >>", resolvedPermalink);
